...
class VisualizeDynamics:
    ...
    def _get_transition(state, action, states_visited, model):
        ...
        if (state, action) not in model:
            ...
            return "Learned Model: State %d    x    Action %d    =>    %s" % (state_id, action, s), new_states
    ...
    def visualize_episode(self, episode_id, episode_return, episode, states_visited, model):
        ...
        for i, action in enumerate(actions):
            ...
            self.axarr[0].text(5, -30, "Episode ID: %d,    Explore-Return: %r,    Num states: %d" %
            ...
            if special_case:
                ...
                self.axarr[0].text(5, -10,
                                   "State %d %s    x    Action %d    =>    State %d %s               Other state %d %s" %
                                   (curr_state_id, curr_state_label, action, next_state_id, next_state_label,
                                    other_state_id, other_state_label))

            else:
                self.axarr[0].text(5, -10, "State %d %s    x    Action %d    =>    State %d %s" %
                                   (curr_state_id, curr_state_label, action, next_state_id, next_state_label))
    ...
    def train(self, experiment, env, env_name, experiment_name, logger, use_pushover,
              debug, do_reward_sensitive_learning=False):
        ...
        for it in range(1, self.max_iter + 1):
            ...
            if it % 10 == 0:
                ...
                logger.log("Episodes %d, Number of states visited %d, Number of state action pair %d, "
                           "Max return %f, Mean return %f, Time taken %f sec"
                           % (it, len(states_visited), len(state_action_visited), max_return, mean_return, time_taken))
                print("Episodes %d, Number of states visited %d, Number of state action pair %d, "
                      "Max return %f, Mean return %f, Time taken %f sec"
                      % (it, len(states_visited), len(state_action_visited), max_return, mean_return, time_taken))
