...
class PixelNoise(ExoUtil):
        ...
        for pixel in self.pixels:
            for i in range(pixel.x - self.size // 2, pixel.x + self.size // 2 + 1):
                for j in range(pixel.y - self.size // 2, pixel.y + self.size // 2 + 1):
                    if 0 <= i < img.shape[0] and 0 <= j < img.shape[1] and \
                            np.max(img[i, j, :]) < 100:      # Pixel is black in color denoting background
                        img[i, j, :] = pixel.color
    ...
    def get_reward(self):
        ...
        if random.random() > 1 / float(self.horizon):
            ...
            score = 0.0
            for pixel in self.pixels:
                score_ = pixel.x / float(self.width) + pixel.y / float(self.height) + \
                        pixel.color[0] / 255.0 + pixel.color[1] / 255.0 + pixel.color[2] / 255.0
                score += (score_ / 5.0)
...
class DropNoise(ExoUtil):
    ...
    def get_reward(self):
        ...
        if random.random() > 1 / float(self.horizon):
            ...
            score = 0.0
            for drop in self.drops:
                score_ = drop.x / float(self.width) + drop.y / float(self.height) + \
                        drop.color[0] / 255.0 + drop.color[1] / 255.0 + drop.color[2] / 255.0
                score += (score_ / 5.0)
