...
class MakeEnvironment:
    ...
    def __init__(self):
        ...
        self.all_names = list(self.rl_acid_names.keys()) + \
                         self.openai_names + \
        ...
        if len(duplicates) > 0:
            raise AssertionError("Environemt names %s, are duplicated across different runs. This confuses the code"
        ...
        for fname in fnames:
            ...
            for name, obj in inspect.getmembers(module):
                ...
                if inspect.isclass(obj) and \
                        str(obj).startswith("<class '%s" % fname) and \
                        issubclass(obj, RLAcidWrapper):
    ...
    def _get_matterport_envs():
        ...
        for fname in fnames:
            ...
            for name, obj in inspect.getmembers(module):
                ...
                if inspect.isclass(obj) and \
                        str(obj).startswith("<class '%s" % fname):      # TODO add MatterportWrapper
    ...
    def _get_control_envs():
        ...
        for fname in fnames:
            ...
            for name, obj in inspect.getmembers(module):
                ...
                if inspect.isclass(obj) and \
                        str(obj).startswith("<class '%s" % fname):      # TODO add control wrapper
    ...
    def _get_mingrid_envs():
        ...
        for fname in fnames:
            ...
            for name, obj in inspect.getmembers(module):
                ...
                if inspect.isclass(obj) and \
                        str(obj).startswith("<class '%s" % fname):      # TODO add Minigrid wrapper
    ...
    def make(self, exp_setup):
        ...
        elif (env_name.startswith("control/") and base_env_name in self.control_names) \
                or env_name in self.control_names:
            return self.control_names[base_env_name](exp_setup.config)

        elif (env_name.startswith("minigrid/") and base_env_name in self.minigrid_names) \
        ...
        elif (env_name.startswith("matterport/") and base_env_name in self.matterport_names) \
