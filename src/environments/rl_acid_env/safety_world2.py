...
class SafetyWorld(RLAcidWrapper):
    ...
    def __init__(self, config):
        ...
        self.warmup_horizon = 5
        assert self.horizon >= self.warmup_horizon
    ...
    def transition(self, x, a):
        ...
        if ix == 0:
            ...
            if a == self.unsafe_actions[h]:     # unsafe action
                return 0, h + 1
            elif a == self.switch_actions[h] and h == 0:   # switch action
                # In safetyworld 2, the agent can only switch to the fast route at the first time step
                return 1, h + 1
            elif a == self.stop_action:         # stop/safe action
                return 3, h + 1
            else:
    ...
    def get_safety_ftr(self, x, a):
        ...
        if ix == 0:     # Safety features are constant
            ...
            if h <= self.warmup_horizon:
                safe_ftr, unsafe_ftr = self.constant_safe_feature, self.constant_unsafe_feature
            else:
    ...
    def safety_query(self, safety_ftr):
