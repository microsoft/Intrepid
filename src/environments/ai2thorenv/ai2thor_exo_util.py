...
class AI2ThorExoUtil:
    ...
    def generate(self, img):
        ...
        draw.ellipse(
            (
                self.circle_coord[0] - self.circle_rad,
                self.circle_coord[1] - self.circle_rad,
                self.circle_coord[0] + self.circle_rad,
                self.circle_coord[1] + self.circle_rad,
            ),
        ...
        draw.polygon(
            [
                (self.triangle_x, self.triangle_y),
                (int(0.15 * self.width) + self.triangle_x, self.triangle_y),
                (
                    int(0.075 * self.width) + self.triangle_x,
                    int(0.075 * self.height) + self.triangle_y,
                ),
            ],
