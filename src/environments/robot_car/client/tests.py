...
def test_image_load(input_dir, camera_count=2):
    ...
    # compare results
    assert cs_array.shape == pil_array.shape, f"Shape of CarState result {cs_array.shape} does not match PIL result {pil_array.shape}"
    assert cs_array.max() == pil_array.max(), f"Max of CarState result {cs_array.max()} does not match PIL result {pil_array.max()}"
    assert cs_array.min() == pil_array.min(), f"Min of CarState result {cs_array.min()} does not match PIL result {pil_array.min()}"
    ...
    error.external_pics = [np.abs(cs.external_pics[0] - pil_state.external_pics[0]), np.abs(cs.external_pics[1] - pil_state.external_pics[1])]
...
def test_pil_vs_cv(start_state_dir, goal_state_dir, k, desired_action, camera_count=2):
    ...
    print(f"Distance: {np.linalg.norm(model_cv._normalize_action(predicted_action_cv) - model_pil._normalize_action(predicted_action_pil))}")
    print()
...
def test_alex_model(start_state_dir, goal_state_dir, k, desired_action, camera_count=2, normalized=False, reconstruct_output=None):
...
if __name__ == "__main__":
    ...
    test_alex_model(os.path.join("test_data", "state_1"), os.path.join("test_data", "state_2"), 1, action_state_1_to_2, reconstruct_output="test_data/reconstruction_1_to_2.jpg")
    test_alex_model(os.path.join("test_data", "state_3"), os.path.join("test_data", "state_4"), 1, action_state_3_to_4, reconstruct_output="test_data/reconstruction_3_to_4.jpg")
    test_alex_model(os.path.join("test_data", "state_5"), os.path.join("test_data", "state_6"), 1, action_state_5_to_6, reconstruct_output="test_data/reconstruction_5_to_6.jpg")
    test_alex_model(os.path.join("test_data", "state_train_1"), os.path.join("test_data", "state_train_2"), 1, action_state_train_1_to_2, reconstruct_output="test_data/reconstruction_train_1_to_2.jpg")
    test_alex_model(os.path.join("test_data", "state_train_3"), os.path.join("test_data", "state_train_4"), 1, action_state_train_3_to_4, reconstruct_output="test_data/reconstruction_train_3_to_4.jpg")
    test_alex_model(os.path.join("test_data", "state_train_5"), os.path.join("test_data", "state_train_6"), 1, action_state_train_5_to_6, reconstruct_output="test_data/reconstruction_train_5_to_6.jpg")
