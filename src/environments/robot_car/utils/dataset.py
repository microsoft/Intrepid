...
class CarDataset(Dataset):
    def __init__(
        self, root_dir, max_k=1, resize=(256, 256), median_filter=False, load_into_memory=False, cache_into_memory=False
    ):
        ...
        subdirs = [
            ...
            if os.path.isdir(os.path.join(root_dir, d)) and os.path.isfile(os.path.join(root_dir, d, "actions.txt"))
        ]
        ...
        for dir in subdirs:
            ...
            for line in log:
                traj_actions.append(
                    normalize_action(
                        [line["angle"], 0.0 if line["direction"] == "forward" else 1.0, line["speed"], line["time"]]
                    )
                )
                traj_pics.append(
                    [
        ...
        if self.load_into_memory:
            ...
            self.pics = torch.empty(
                (self.pic_filenames.shape[0], self.pic_filenames.shape[1], 3, self.resize[0], self.resize[1]),
                dtype=torch.uint8,
        ...
        if self.cache_into_memory:
            ...
            self.pic_is_cached = torch.zeros(
                self.pic_filenames.shape[0], dtype=torch.uint8, device=torch.device("cpu")
            ).share_memory_()
