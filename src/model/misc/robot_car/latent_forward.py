...
class LatentForward(pl.LightningModule):
    def __init__(self, input_dim=512, latent_dim=64, action_dim=4, kl=0.0001, lr=1e-4, vae=None):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(3*input_dim, 2048), nn.BatchNorm1d(2048), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(2048, 1024), nn.BatchNorm1d(1024), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(1024, 512), nn.BatchNorm1d(512), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(512, 256), nn.BatchNorm1d(256), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(256, 2*latent_dim)
        )
        self.action_encoder = nn.Sequential(
            nn.Linear(action_dim, latent_dim), nn.BatchNorm1d(latent_dim), nn.LeakyReLU(),
            nn.Linear(latent_dim, latent_dim)
        )
        self.decoder = nn.Sequential(
            nn.Linear(2*latent_dim, 256), nn.BatchNorm1d(256), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(256, 512), nn.BatchNorm1d(512), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(512, 1024), nn.BatchNorm1d(1024), nn.LeakyReLU(), nn.Dropout(d),
            nn.Linear(1024, 2048), nn.BatchNorm1d(2048), nn.LeakyReLU(), nn.Dropout(d),
    ...
    def _run_step(self, batch, batch_idx, run_type="train"):
        ...
        state, state_next, action, mask = self.augment(state, state_next, action, jitter=(run_type=="train"), replace=(0.2 if run_type=="train" else 0.0))
        ...
        if batch_idx == 0 and run_type == "val" and self.vae is not None:
            ...
            save_image(output[:8*num_img_per_row], os.path.join(image_out_dir, f"{run_type}_latent_fwd.jpg"), nrow=num_img_per_row)
            if run_type == "val":
...
if __name__ == "__main__":
    ...
    trainer = pl.Trainer(default_root_dir=train_root, callbacks=[checkpoint_callback], max_epochs=1000, num_sanity_val_steps=1, check_val_every_n_epoch=5)
