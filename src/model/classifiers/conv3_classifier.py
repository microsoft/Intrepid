...
class Conv3Classifier(nn.Module):
    ...
    def __init__(self, num_class, config, constants, bootstrap_model=None):
        ...
        if config["feature_type"] == "feature":
            ...
            self.image_conv = nn.Sequential(
                nn.Conv2d(self.channel + 4, 16, (8, 8), 4),
                nn.ReLU(),
                nn.Conv2d(16, 32, (4, 4), 2),
                nn.ReLU(),
                nn.Conv2d(32, 32, (4, 4), 2),
                nn.ReLU(),
                nn.Flatten(),
                nn.Linear(288, num_class),
            )
    ...
    def save(self, folder_name, model_name=None):
        if model_name is None:
            torch.save(self.state_dict(), folder_name + Conv3Classifier.NAME)
        else:
            torch.save(self.state_dict(), folder_name + model_name)

    def load(self, folder_name, model_name=None):
        if model_name is None:
            self.load_state_dict(torch.load(folder_name + Conv3Classifier.NAME))
        else:
