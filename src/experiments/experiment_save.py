...
def terminate(performance, exp_setup, seeds):
    ...
    if len(performance) > 0:
        ...
        for key in performance[0]:  # Assumes the keys are same across all runes
            ...
            exp_setup.logger.log("%r: Mean %f, Std %f, Median %f, Min %f, Max %f, Num runs %d, All performance %r" %
                                 (key, np.mean(metrics), np.std(metrics),  np.median(metrics),
                                  np.min(metrics), np.max(metrics), len(metrics), metrics))
