...
class PPEDebugger:
    ...
    def _calc_gap(i, j, dataset, state_stats, path_map):
        ...
        for dp in dataset:
            ...
            gap += abs(
                state_stats[endo_state].get_prob_entry(path_map[i].path_id)
    ...
    def _save_fig(self, h, dataset, path_common, path_map):
        ...
        for path_id in range(0, len(path_map)):
            parent_path_id, action = (
                path_map[path_id].parent_path_id,
                path_map[path_id].action,
            )
            for dp in dataset:
                if dp[1] == path_id:
                    ...
                    if path_id in path_common:
                        imageio.imwrite(
                            "%s/selected_path_number_%d.png" % (base_folder, path_id),
                            img,
                        )

                        imageio.imwrite(
                            "%s/parent_%d/action_%d_path_number_%d_selected.png"
                            % (base_folder, parent_path_id, action, path_id),
                            img,
                        )
                    else:
    ...
    def _compute_dynamics_error(
        ...
        for i in range(0, len(path_map)):
            ...
            for j in range(i + 1, len(path_map)):
                ...
                if result == ErrorUtil.ERROR1:
                    ...
                    self.logger.log(
                        "[Error 1] Path %s (%s) merged with path %s (%s). "
                    ...
                    self.logger.log(
                        "[Error 2] Path %s (%s) was not merged with path %s (%s). "
        ...
        for ctr, (state, state_prob) in enumerate(state_stats.items()):
            ...
            self.logger.log(
                "%d. State: %r has %d many paths reaching it in new policy cover."
                % (
                    ctr,
                    state,
                    len([path_id for path_id in paths if path_id in path_common]),
                )
            )
