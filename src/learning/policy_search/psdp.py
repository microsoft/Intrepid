...
class PSDP(AbstractPolicySearch):
    ...
    def do_train(config, constants, homing_policy_dataset, env_info, policy_folder_name, actions, horizon,
        ...
        if encoder_reward_args is None:
            ...
            reward_func = lambda obs, h: 1 if h == horizon and \
                                              encoding_function.encode_observations(obs) == reward_id else 0

        learned_policy, mean_reward, _ = psdp.train(homing_policy_dataset, env, actions, horizon, reward_func,
                                                    homing_policies, logger, None, debug)
    ...
    def train(self, replay_memory, env, actions, horizon, reward_func, homing_policies, logger,
              tensorboard, debug=False):
        """ Performs the learning """

        time_start, sample_time, oracle_time,  = time.time(), 0, 0
        learned_policy = dict()
        ...
        for step in range(horizon, 0, -1):
            ...
            contextual_bandit_dataset = [
                self._gather_sample(env, actions, horizon, step, homing_policies, learned_policy, reward_func)
            ...
            if debug:
                logger.log("Step %d CB: Best Test Loss %f, Best Train Loss %f, and Stopping max_epoch %d" %
                           (step, round(info["cb-best-test-loss"], 4),
                            round(info["cb-best-train-loss"], 4), info["stopping_epoch"]))
                self.log_intermediate_results(contextual_bandit_dataset, optimal_policy_step, env, actions, horizon,
                                              step, homing_policies, learned_policy, reward_func, logger,
                                              self.num_eval_samples, self.constants["cb_oracle_batch_size"])
        ...
        logger.log("PSDP cost is %r sec. {Samples: %r, CB-Oracle: %r}" %
                   (psdp_step_time, (sample_time * 100.0) / psdp_step_time, (oracle_time * 100.0) / psdp_step_time))

        total_episodes = horizon * self.num_train_samples

        return learned_policy, mean_reward, {"sum_rewards": sum_rewards, "total_episodes": total_episodes}
