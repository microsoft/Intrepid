...
class GeneralizedInverseKinematics:
    ...
    def calc_loss(self, model, batch, epoch, discretized, test_set_errors=None, past_entropy=None):
        ...
        past_observations = cuda_var(torch.cat([torch.from_numpy(np.array(point.get_curr_obs())).view(1, -1)
                                                for point in batch], dim=0)).float()
        past_actions = cuda_var(torch.cat([torch.from_numpy(np.array(point.get_action())).view(1, -1)
                                           for point in batch], dim=0)).long()
        observations = cuda_var(torch.cat([torch.from_numpy(np.array(point.get_next_obs())).view(1, -1)
                                           for point in batch], dim=0)).float()

        # Compute loss
        log_probs, meta_dict = model.gen_log_prob(prev_observations=past_observations,
                                                  observations=observations,
                                                  discretized=discretized)  # outputs of size batch x num_actions
