    ...
    def calc_feature(self, observation):
        ...
        for i in range(0, self.dim, self.batch_size):
            ...
            prev_observations = cuda_var(
                torch.cat([torch.from_numpy(np.array(curr_obs_action_[0])).view(1, -1)
                           for curr_obs_action_ in self.curr_obs_actions[i:i + this_batch_size]], dim=0)).float()

            actions = cuda_var(
                torch.cat([torch.from_numpy(np.array(curr_obs_action_[1])).view(1, -1)
                           for curr_obs_action_ in self.curr_obs_actions[i:i + this_batch_size]], dim=0)).long()

            observations = cuda_var(torch.cat([next_obs_np] * this_batch_size, dim=0)).float()

            prob, _ = self.model.gen_prob(prev_observations=prev_observations,
                                          actions=actions,
...
class CompositionalFeatureComputation:
    ...
    def __init__(self, curr_obs_actions, model, batch_size, discretized, compose=True):
        ...
        for i in range(0, self.dim, self.batch_size):
            ...
            prev_obs_act_encoded = self.model.encode_prev_obs_action(prev_observations=prev_observations,
                                                                     actions=actions).detach()

            self.prev_obs_act_encoded_list.append(prev_obs_act_encoded)

        print("Time taken to encode previous action and observation is ", time.time() - time_obs_act_start)
    ...
    def _calc_feature_compose(self, observations):
        ...
        for i in range(0, num_obs, self.batch_size):
            ...
            obs_batch = cuda_var(
                torch.cat([torch.from_numpy(np.array(obs_)).view(1, -1)
                           for obs_ in observations[i:i + this_batch_size]], dim=0)).float()
