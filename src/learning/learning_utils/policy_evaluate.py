...
def generate_failure_result(env, train_samples, regret=True):
    ...
    if optimal_policy_v is None or not regret:
        ...
        return {"train_episodes": train_samples,
                "test_episodes": 0,
...
def evaluate_for_half_regret(env, policy, horizon, optimal_policy_v, logger,
                             train_episodes, sum_train_reward, max_episodes=500000):
    ...
    for test_episode in range(1, max_episodes + 1):
        ...
        if test_episode % 10000 == 0:
            logger.log("(Total Episodes %d) Total reward %r (Optimal) vs %r (System). Ratio %r" % (
                train_episodes + test_episode,  opt_policy_cumm_ret, policy_cumm_ret, ratio))
