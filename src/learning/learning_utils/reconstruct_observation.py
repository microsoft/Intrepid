...
class ReconstructObservation:
    ...
    def train(self, encoder, dataset, tensorboard=None):
        ...
        time_start = time.time()
        self.logger.log("Train/Test = %d/%d; Learning Rate %f, Max Epochs %d, Patience %d" %
                        (len(train_dataset), len(test_dataset), self.learning_rate, self.max_epoch, self.patience))
        ...
        for epoch in range(1, self.max_epoch + 1):
            ...
            train_batches = [train_dataset[i:i + self.batch_size]
            ...
            self.logger.debug("Epoch %d: Train Loss %.4f, Test Loss %.4f, Time taken %s" %
        ...
        self.logger.log("AutoEncoder Trained [Time Taken %s], Best Tune Loss %.4f at max_epoch %d, "
                        "Train Loss after %d epochs is %.4f " %
                        (elapsed_from_str(time_start), best_test_loss, best_epoch, epoch, train_loss))
    ...
    def reconstruct(self, encoder, decoder, test_dataset, base_folder, max_generate=100):
        ...
        self.logger.log("Reconstruction: Total observations %d, Mean loss %d, Time taken %s" %
